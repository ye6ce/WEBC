...
// Persistence Helpers
const STORAGE_PREFIX = 'lumina_v1_';
const saveToLocal = (key: string, data: any) => localStorage.setItem(`${STORAGE_PREFIX}${key}`, JSON.stringify(data));
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Persistence Syncing - Whenever these states change, save them globally
  ...
  const updateTheme = (newT: Partial<StoreTheme>) => { setTheme(prev => ({ ...prev, ...newT })); notify("Design sauvegardé."); };
  ...
  const updateDeliveryPrices = (prices: WilayaDeliveryPrice[]) => { setDeliveryPrices(prices); notify("Frais de livraison sauvegardés."); };
