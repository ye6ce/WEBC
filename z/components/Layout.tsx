      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-10 shadow-2xl relative animate-in zoom-in duration-300">
         ...
         <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
               ...
               <input 
                  value={user} 
                  onChange={e => handleInputChange(setUser, e.target.value)} 
                  className={`w-full p-5 bg-slate-50 border-2 rounded-2xl font-bold text-sm outline-none transition-all ${error ? 'border-red-200 focus:border-red-500' : 'border-slate-100 focus:border-indigo-500'}`} 
            ...
            <div className="space-y-2">
               ...
               <input 
                  type="password" 
                  value={pass} 
                  onChange={e => handleInputChange(setPass, e.target.value)} 
                  className={`w-full p-5 bg-slate-50 border-2 rounded-2xl font-bold text-sm outline-none transition-all ${error ? 'border-red-200 focus:border-red-500' : 'border-slate-100 focus:border-indigo-500'}`} 
                  placeholder="••••••••" 
               />
            </div>
            <div className="space-y-4 pt-2">
              <button type="submit" className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-black active:scale-95 transition-all">Accéder au Dashboard</button>
              {error && (
                <p className="text-red-500 text-[10px] font-black uppercase text-center tracking-widest animate-in slide-in-from-top-2 duration-300">
                  {error}
                </p>
              )}
            </div>
