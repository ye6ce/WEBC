                   ...
                   <div className="flex flex-wrap justify-center gap-4">
                      ...
                      {formData.images.length < 4 && (
                        <label className="w-20 h-20 rounded-2xl border-4 border-dashed border-slate-100 flex items-center justify-center text-slate-200 cursor-pointer hover:bg-slate-50 transition-all font-black text-2xl">
                           + <input type="file" multiple onChange={async e => { if(e.target.files) { const files = Array.from(e.target.files); const base64s = await Promise.all(files.map(f => fileToBase64(f))); setFormData({...formData, images: [...formData.images, ...base64s].slice(0, 4)}); } }} className="hidden" />
                        </label>
                      )}
                   </div>
                </div>
